  ...
  <div className="w-full min-h-screen bg-linear-to-br from-[#F7F3EE] via-[#F7F3EE] to-[#F7F3EE] text-zinc-800 font-sans">
      ...
      <nav className="w-full py-4 bg-white/80 shadow-sm sticky top-0 z-20 animate-fade-down">
        ...
        <ul className="hidden md:flex gap-6 text-lg font-medium justify-center">
          {menu.map((item) => (
            <li key={item.href}>
              <Link href={item.href} className={`px-4 py-2 rounded-xl transition-colors duration-200 ${pathname === item.href ? 'bg-[#F7F3EE] text-[#6b7280] font-bold shadow' : 'hover:bg-[#e6efe2] hover:text-[#6b7280]'}`}>{item.name}</Link>
            </li>
          ))}
        </ul>
        {/* Mobile sidebar menu */}
            ...
            >
              ...
              >
                <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <line x1="7" y1="7" x2="21" y2="21" stroke="#e6efe2" strokeWidth="2" strokeLinecap="round" />
                  <line x1="21" y1="7" x2="7" y2="21" stroke="#e6efe2" strokeWidth="2" strokeLinecap="round" />
                </svg>
              </button>
              <ul className="flex flex-col gap-4 text-lg font-medium">
                {menu.map((item) => (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      className={`block px-4 py-2 rounded-xl transition-colors duration-200 ${pathname === item.href ? 'bg-[#F7F3EE] text-[#6b7280] font-bold shadow' : 'hover:bg-[#6b7280] hover:text-[#6b7280]'}`}
                      onClick={() => setOpen(false)}
                    >
                      {item.name}
                    </Link>
      ...
      <footer className="py-8 px-4 text-center text-zinc-500 animate-fade-up">
        ...
        <p className="text-base mb-2">Un día, una historia, un amor eterno. Gracias por ser parte de nuestro sueño.</p>
        <p className="text-base">© 2025 Carla & Alejandro · Todos los derechos reservados · <Link href="/rsvp" className="underline">Política de privacidad (RGPD)</Link></p>
      </footer>
    </div>
  );
}
